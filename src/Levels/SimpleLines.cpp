...
namespace SimpleLines {
	...
	void Demo3()
	{
		AEVec2 center = { float(AEGetSysWindowWidth() / 2), float(AEGetSysWindowHeight() / 2) };

		for (float y = 100; y >= -100; --y) {
			Rasterizer::DrawLine(AEVec2(0, center.y + y), Rasterizer::Color(1, 0, 0, 1),
				AEVec2((float)AEGetSysWindowWidth(), center.y + y), Rasterizer::Color(0, 0, 1, 1));
		}
	}
	
	void Demo4()
	{
		float step = TWO_PI / 90.0f;
		float lenght = 150.0f;
		
		AEVec2 center = { float(AEGetSysWindowWidth() / 2), float(AEGetSysWindowHeight() / 2) };
		AEVec2 p;
		Color c[2] = { {1,0,0,1},{0,0,1,1} };
		for (float a = 0; a <= TWO_PI; a += step) {
			p = AEVec2(cos(a), sin(a)) * lenght;
			Rasterizer::DrawLine(center, c[0], center + p, c[1]);
		}
	}
	...
	void Menu()
	{
		...
		if (ImGui::MenuItem("Line Color Lerp 1")) mode = 2;
		if (ImGui::MenuItem("Line Color Lerp 2")) mode = 3;
	}
}